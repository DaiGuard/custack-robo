...
| | `0x03` | **Crawler (キャタピラ)** | 悪路走破（泥・森の減速大幅カット） / 毒沼・溶岩ダメージ 80% カット | 履帯 PWM |
| **アームユニット** (`0x31`/`0x32`) | `0x01` | **ガトリング** | 拡散黄色弾幕連射（威力 8, 15発/秒, $\pm 7.5^\circ$ 拡散） | 200ms 点滅振動駆動 |
| | `0x02` | **ソード** | 近接高威力・エメラルド三日月スイング（威力 80, 半径 1.875m, CD 0.45s） | 100ms 単発パルス駆動 |
| | `0x03` | **大型レーザーキャノン** | 極大高出力ブルーレーザー（威力 60, 速度 7.5, 寿命 3.5s, CD 1.8s） | 高出力トリガー駆動 |
