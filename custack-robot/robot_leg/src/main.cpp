...
void loop() {
    ...
    //  vx > 0 のとき車輪は逆転せず、常に正回転の範囲内で左右比率を分配
    //  omega = 0     -> R: 100%, L: 100% (直進)
    //  omega = 1000  -> R:  70%, L:  30%
    //  omega = -1000 -> R:  30%, L:  70%
    ...
    if (vx != 0) {
        if (omega >= 0) {
            // omega = 0〜1000 -> R: 100%〜70%, L: 100%〜30%
            speed_r = ((int32_t)vx * (10000 - 3 * (int32_t)omega)) / 10000;
            speed_l = ((int32_t)vx * (10000 - 7 * (int32_t)omega)) / 10000;
        } else {
            // omega = 0〜-1000 -> R: 100%〜30%, L: 100%〜70%
            speed_r = ((int32_t)vx * (10000 + 7 * (int32_t)omega)) / 10000;
            speed_l = ((int32_t)vx * (10000 + 3 * (int32_t)omega)) / 10000;
        }
    }
