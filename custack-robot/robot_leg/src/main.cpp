...
#elif DEVICE_ID == 0x02
    //----------------------------------------------------------------------
    // 差動二輪モード (仮想ステアリング: 前後入力(+vx)がある時のみ旋回)
    // omega (-1000〜1000) に応じて左右輪への vx 割合をダイナミックに分配
    // PWM1: 右輪 (正回転 DIR=+1), PWM3: 左輪 (逆回転 DIR=-1)
    //----------------------------------------------------------------------
    int32_t speed_r = 0;
    int32_t speed_l = 0;

    // 前後入力 (vx != 0) がある場合のみ、omega に応じて内輪・外輪の速度割合を分配
    if (vx != 0) {
        if (omega >= 0) {
            // 右旋回: 外輪(左輪) = vx, 内輪(右輪) = vx * (1000 - 2 * omega) / 1000
            //  omega = 0    -> L: vx, R: vx     (直進)
            //  omega = 500  -> L: vx, R: 0      (片輪旋回)
            //  omega = 1000 -> L: vx, R: -vx    (超信地急旋回)
            speed_l = (int32_t)vx;
            speed_r = ((int32_t)vx * (1000 - 2 * (int32_t)omega)) / 1000;
        } else {
            // 左旋回: 外輪(右輪) = vx, 内輪(左輪) = vx * (1000 - 2 * |omega|) / 1000
            //  omega = 0    -> R: vx, L: vx     (直進)
            //  omega = -500 -> R: vx, L: 0      (片輪旋回)
            //  omega = -1000-> R: vx, L: -vx    (超信地急旋回)
            speed_r = (int32_t)vx;
            speed_l = ((int32_t)vx * (1000 + 2 * (int32_t)omega)) / 1000;
        }
    }
